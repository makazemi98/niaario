...
export default {
  ...
  methods: {
    ...
    getData() {
      axios
        .get(`/admin/suppliers/${this.$route.params.id}`)
        .then((response) => {
          this.form = response.data.data;
          this.form.customers_id = this.extractValue(
            response.data.data.customers,
            "id"
          );
          this.form.persons_in_charge = this.extractValue(
            response.data.data.persons_in_charge,
            "id"
          );
          this.form.supplying_brands = this.extractValue(
            response.data.data.supplying_brands,
            "name"
          );
          this.form.product_categories = this.extractValue(
            response.data.data.product_categories,
            "title"
          );
        });
      this.isLoaded = true;
    ...
    submit() {
      this.$refs.createSupplierForm.validate().then((success) => {
        if (success) {
          ...
          axios({
            method: "PUT",
            url: `/admin/suppliers/${this.$route.params.id}`,
            data: this.form,
          })
            .then((response) => {
              this.loading = false;
